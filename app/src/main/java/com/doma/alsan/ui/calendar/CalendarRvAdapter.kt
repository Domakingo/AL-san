...
import com.doma.alsan.databinding.ListCalendarBinding
    ...
    inner class ItemViewHolder(private val binding: ListCalendarBinding) : ViewHolder(binding) {
        override fun bind(item: AiringSchedule, index: Int) {
            with(binding) {
                ...
                calendarMediaListStatusText.text = item.media.mediaListEntry?.status?.getString(item.media.type?.getMediaType() ?: com.doma.alsan.helper.enums.MediaType.ANIME)?.uppercase()
